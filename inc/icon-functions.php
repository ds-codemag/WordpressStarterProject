...
function include_svg_icons() {
	...
	$svg_icons = get_parent_theme_file_path( '/assets/images/sprite.svg' );
...
function get_svg( $args = array() ) {
	...
	if ( empty( $args ) ) {
		return __( 'Please define default parameters in the form of an array.', 'basetheme' );
	}

	// Define an icon.
	if ( false === array_key_exists( 'icon', $args ) ) {
		return __( 'Please define an SVG icon filename.', 'basetheme' );
	}
...
add_filter( 'nav_menu_item_title', 'dropdown_icon_to_menu_link', 10, 4 );
...
function social_links_icons() {
	...
	/**
	 * Filter template social links icons.
	 */
